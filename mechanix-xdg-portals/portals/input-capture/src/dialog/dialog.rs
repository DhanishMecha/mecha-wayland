use crate::backend::types::{InputCaptureOutcome, InputCaptureResponse, RequestHandle};
...
impl WidgetList for InputCaptureDialogUi {
    ...
    fn on_event(&mut self, ctx: &mut EventCtx) {
        ...
        if self.confirm_rect != utils::Rect::ZERO && interactivity.is_clicked(self.confirm_rect) {
            println!("[input-capture-ui] Confirmed for handle={}", self.handle);
            ctx.dispatch(InputCaptureResponse {
                handle: self.handle.clone(),
                outcome: InputCaptureOutcome::Granted {
        ...
        if self.cancel_rect != utils::Rect::ZERO && interactivity.is_clicked(self.cancel_rect) {
            println!("[input-capture-ui] Cancelled for handle={}", self.handle);
            ctx.dispatch(InputCaptureResponse {
                handle: self.handle.clone(),
                outcome: InputCaptureOutcome::Denied,
            });
            return;
        }
    }
...
fn make_root(
    ...
    let subtitle_str = if app_id.is_empty() {
        "An application wants to capture input.".to_string()
    } else {
    ...
    body_text1.text = l1;
    body_text1.color = Color::rgb(0.75, 0.75, 0.82);
    body_text1.z = 0.95;

    let mut body_text2 = Text::new(Style::default());
    body_text2.font = Some(font_16);
    body_text2.text = l2;
    body_text2.color = Color::rgb(0.75, 0.75, 0.82);
    body_text2.z = 0.95;

    let body_div = Div::new(body_style, (body_text1, body_text2));
    ...
    let mut modal = Div::new(modal_style, (header, body_div, button_row));
...
fn wrap_text(text: &str, max_chars: usize) -> (String, String) {
    ...
    for word in text.split_whitespace() {
        if !reached_limit && char_count + word.len() + 1 <= max_chars {
            if !line1.is_empty() {
                line1.push(' ');
                char_count += 1;
            }
