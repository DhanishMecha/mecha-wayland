use crate::backend::{DeviceType, RemoteDesktopOutcome, RemoteDesktopResponse, RequestHandle};
...
pub struct RemoteDesktopDialogUi {
    handle: RequestHandle,
    /// Device types the calling application requested (bitmask of `DeviceType`).
    device_types: u32,
    root: RootDiv,
...
impl RemoteDesktopDialogUi {
    pub fn new(handle: RequestHandle, app_id: String, device_types: u32) -> Self {
        let font_24 = &atlas::UI_FONT_INTER_24;
        let font_16 = &atlas::UI_FONT_INTER_16;

        let device_label = device_type_label(device_types);
        let body = format!(
            "Allow \"{}\" to remotely control your desktop?\n\nRequested access: {}",
            app_id, device_label
        );
...
impl WidgetList for RemoteDesktopDialogUi {
    ...
    fn on_event(&mut self, ctx: &mut EventCtx) {
        ...
        if self.confirm_rect != utils::Rect::ZERO && interactivity.is_clicked(self.confirm_rect) {
            println!("[remote-desktop-ui] Confirmed for handle={}", self.handle);
            ctx.dispatch(RemoteDesktopResponse {
                handle: self.handle.clone(),
                outcome: RemoteDesktopOutcome::Granted {
                    granted_devices: self.device_types,
                },
            });
            return;
        }
        ...
        if self.cancel_rect != utils::Rect::ZERO && interactivity.is_clicked(self.cancel_rect) {
            println!("[remote-desktop-ui] Denied for handle={}", self.handle);
            ctx.dispatch(RemoteDesktopResponse {
                handle: self.handle.clone(),
                outcome: RemoteDesktopOutcome::Denied,
            });
        }
    }
...
/// Build a human-readable label from the requested device bitmask.
fn device_type_label(types: u32) -> String {
    let mut parts = Vec::new();
    if types & DeviceType::Keyboard as u32 != 0 {
        parts.push("Keyboard");
    }
    if types & DeviceType::Pointer as u32 != 0 {
        parts.push("Pointer");
    }
    if types & DeviceType::Touchscreen as u32 != 0 {
...
fn make_root(
    ...
    let subtitle_str = if app_id.is_empty() {
        "An application wants to control your desktop.".to_string()
    } else {
